...
export async function POST(request: NextRequest) {
  try {
    ...
    const createdStartup = await prisma.$transaction(async (prisma) => {
      const startup = await prisma.startups.create({
        data: {
          ...
          has_patent: data.hasPatent || null,
          patent_and_code: data.patentAndCode || null,
          ...
          is_there_partners_agreement_signed:
            data.isTherePartnersAgreementSigned,
