...
import { DataExpertsSignUpSchema } from "@/lib/schema-experts";

export default function DataExpertsSignUpUser() {
  ...
  const formSchema = DataExpertsSignUpSchema(t);
  ...
  function onHandleFormSubmit(data: z.infer<typeof formSchema>) {
    ...
    startTransition(() => {
      checkEmailExists(
        data.emailSignUp,
        t("experts-form.experts-form-email-already-exists"),
        UserType.EXPERT
      )
        .then((values) => {
          if (!values.error) {
    ...
    <form className="space-y-6" onSubmit={handleSubmit(onHandleFormSubmit)}>
      ...
      <div className="flex flex-col gap-1 text-xs lg:text-base">
        ...
        <label htmlFor="emailSignUp" className="flex items-center mt-5">
          <div className="flex flex-col">
            <div>
              <span>{t("experts-form.experts-form-sign-up.email-field")}</span>
              <span className="text-red-500 ml-1">*</span>
            </div>
          </div>
        </label>
        ...
        <label htmlFor="passwordSignUp" className="flex items-center mt-5">
          <div className="flex flex-col">
            <div>
              <span>
                {t("experts-form.experts-form-sign-up.password-field")}
              </span>
              <span className="text-red-500 ml-1">*</span>
            </div>
          </div>
        ...
        >
          <div className="flex flex-col">
            <div>
              <span>
                {t("experts-form.experts-form-sign-up.confirm-password-field")}
              </span>
              <span className="text-red-500 ml-1">*</span>
            </div>
          </div>
      ...
      <div className="flex justify-between">
        ...
        >
          {t("experts-form.experts-form-previous-button")}
        </Button>
        <Button
          disabled={isPending}
          variant="blue"
          className="px-6 text-white rounded-md"
        >
          {t("experts-form.experts-form-next-button")}
        </Button>
      </div>
    </form>
  );
